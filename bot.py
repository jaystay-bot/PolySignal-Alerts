...
KALSHI_REFERRAL_URL = os.environ.get("KALSHI_REFERRAL_URL", "https://kalshi.com")
...
async def fetch_market_detail(market_id: str) -> dict | None:
...
def compute_signal(market: dict) -> dict | None:
    try:
        ...
        yes_price = float(market.get("outcomePrices", "[0,0]").strip("[]").split(",")[0])
    ...
    window_close = now.strftime("%H:%M UTC")
...
async def check_followups():
    ...
    for sig in due:
        ...
        detail = await fetch_market_detail(sig["market_id"])
        if detail is None:
            continue
...
async def main():
    ...
    scheduler.add_job(scan_markets, "interval", minutes=SCAN_INTERVAL_MINUTES, next_run_time=datetime.now(timezone.utc))
    scheduler.add_job(check_followups, "interval", minutes=1)
    scheduler.start()

    log.info(f"Scheduled: scan every {SCAN_INTERVAL_MINUTES}m, follow-ups checked every 1m.")
